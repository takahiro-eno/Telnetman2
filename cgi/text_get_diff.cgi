...
if(defined($sjis) && ($sjis eq '1')){
 $charset = 'Shift_JIS';
 $file_diff_log = &Common_system::file_diff_log_sjis($session_id, $node);
}
else{
 $charset = 'UTF-8';
 $file_diff_log = &Common_system::file_diff_log($session_id, $node);
}

unless((length($file_diff_log) > 0) && (-f $file_diff_log)){
 print "Content-type: text/plain; charset=UTF-8\n\n";
 print 'login:1' . "\n";
 print 'session:1' . "\n";
 print 'session_id:' . $session_id . "\n";
